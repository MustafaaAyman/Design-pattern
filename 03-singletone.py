...
OBJECT1 = Singleton() ##: Creates an instance of the Singleton class. Since __new__ method returns a reference to the class itself, OBJECT1 ends up referring to the class Singleton rather than an instance of it.
print(f"id(OBJECT1)\t= {id(OBJECT1)}")

OBJECT2 = copy.deepcopy(OBJECT1)#copy.deepcopy(OBJECT1): Attempts to create a deep copy of OBJECT1. However, since OBJECT1 refers to the class Singleton, OBJECT2 also ends up referring to the class Singleton.
print(f"id(OBJECT2)\t= {id(OBJECT2)}")

OBJECT3 = Singleton()# Creates another instance of the Singleton class. Again, __new__ returns a reference to the class itself, so OBJECT3 also refers to the class Singleton
